...
static inline long tb_run_ticks_stoppable(Testbench *tb, int simlen, bool reset = false)
{
  ...
  std::cout << "RFUZZ max possible coverage: " << kNumCoveragePoints << std::endl;
  std::cout << "RFUZZ achieved coverage: " << tb->get_coverage_amount() << std::endl;
  std::cout << "RFUZZ coverage mask: " << tb->get_coverage_mask() << std::endl;
