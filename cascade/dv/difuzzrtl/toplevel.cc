...
static inline long tb_run_ticks_stoppable(Testbench *tb, int simlen, bool reset = false)
{
  ...
  // Get the coverage
  // std::cout << "Getting coverage." << std::endl;
  std::cout << "DifuzzRTL max possible coverage: " << kNumCoveragePoints << std::endl;
  std::cout << "DifuzzRTL achieved coverage: " << tb->get_coverage_amount() << std::endl;
  std::cout << "DifuzzRTL coverage mask: " << tb->get_coverage_mask() << std::endl;
