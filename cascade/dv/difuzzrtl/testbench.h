...
typedef Vvexriscv_tiny_soc Module;
  ...
  tick_req_t tick(int num_ticks = 1, bool false_tick = false) {
    ...
    for (size_t i = 0; i < num_ticks || num_ticks == -1; i++) {
      ...
      if ((module_->data_mem_req_o & 1) == 1 && (module_->data_mem_we_o & 1) == 1) {
          if ((module_->data_mem_addr_o & 0x7FFFFFFFULL) == 0x00000000ULL) {
            if (ret.type != REQ_NONE)
              std::cerr << "WARNING: Missed a request during a batch of ticks." << std::endl;
            ret.type = REQ_STOP;
          } else if ((module_->data_mem_addr_o & 0x7FFFFFFFULL) == 0x00000010ULL) {
            if (ret.type != REQ_NONE)
              std::cerr << "WARNING: Missed a request during a batch of ticks." << std::endl;
            // std::cout << "module_->data_mem_strb_o = " << std::hex << (int)(module_->data_mem_strb_o) << std::endl;
            ret.type = REQ_INTREGDUMP;
            ret.content = module_->data_mem_wdata_o & expand_mask(module_->data_mem_strb_o);
          } else if ((module_->data_mem_addr_o & 0x7FFFFFFFULL) == 0x00000018ULL) {
            if (ret.type != REQ_NONE)
              std::cerr << "WARNING: Missed a request during a batch of ticks." << std::endl;
            // std::cout << "module_->data_mem_strb_o = " << std::hex << (int)(module_->data_mem_strb_o) << std::endl;
